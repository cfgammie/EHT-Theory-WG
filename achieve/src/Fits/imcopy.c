/*! \file
\brief 
Copy the input FITS image to the new output file with optional data
compression. 
...
Copy the input FITS image to the new output file with optional data
compression. The output image maybe externally compressed with the
gzip or Unix compress algorithm, or it may be internally compressed
with the new tile compression format. The output image may be written
in the tile-compressed FITS image format by adding the compression
qualifiers in square brackets following the output file name.
Examples:
...
This extracts a 500 X 500 pixel section of the much larger input image
(which may be in tile-compressed format).  The output is a normal
